...
abstract class connQakBase(val hostIP : String,   val port : String,   val destName : String,  val ctxdest : String) {
lateinit var currQakConn  : connQakBase
	
	companion object{
	fun create(connType: ConnectionType,   hostIP : String,   port : String,
			   destName : String, ctxdest : String) : connQakBase{
		  showSystemInfo()
		  when( connType ){
				 ConnectionType.MQTT -> 
				 ...
				 ConnectionType.COAP ->
				 	{return connQakCoap(hostIP, port, destName, ctxdest)}  
  				 ConnectionType.HTTP ->  
